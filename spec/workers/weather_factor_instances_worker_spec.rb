...
RSpec.describe WeatherFactorInstancesWorker, type: :worker do
    ...
    describe "#perform" do
      context 'when an occurrence is create' do
        context 'adds the temperature, the humidity and the weather_condition to the occurrence' do
          before(:each) do
              expected_hash = { "date" => {"hour" => "20", "min" => "30"}, "tempm" => "26", "hum" => "60", "conds" => "overcast"}
              allow(@instance).to receive(:get_weather_information) { expected_hash}

              @occurrence = create(:occurrence_with_gps_coordinates)

              @factor1 = create(:factor, id:1, name:"temperature", factor_type:"celsius")
              @factor2 = create(:factor, id:2, name:"humidity", factor_type:"percentage")
              @factor3 = create(:factor, id:3, name:"weather_condition", factor_type:"string")

              @instance.perform(@occurrence.id)
          end
          it 'adds the correct data to the occurrence' do
            @occurrence.factor_instances.each { |factor_instance|
                if factor_instance.factor_id == 1 then
                  expect(factor_instance.value).to eq "26"
                end
                if factor_instance.factor_id == 2 then
                    expect(factor_instance.value).to eq "60"
                end
                if factor_instance.factor_id == 3 then
                    expect(factor_instance.value).to eq "overcast"
                end
            }
          end
        end
